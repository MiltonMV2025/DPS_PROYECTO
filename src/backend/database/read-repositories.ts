...
export type ReadRepositories = {
  ...
  suppliers: { findAll(categories?: string[]): Promise<SupplierListItem[]> };
...
export function createReadRepositories(pool: Pool = getDatabasePool()): ReadRepositories {
  ...
  return {
    ...
    patients: list(`SELECT p.id_paciente, u.nombre, u.correo, u.activo, p.telefono, p.fecha_nacimiento, p.alergias FROM Pacientes p JOIN Usuarios u ON u.id_usuario=p.id_usuario ORDER BY u.nombre`, r => ({ id: Number(r.id_paciente), name: text(r.nombre), email: text(r.correo), phone: text(r.telefono), birthDate: text(r.fecha_nacimiento), allergies: nullable(r.alergias), active: Boolean(r.activo) })),
    inventory: list(`SELECT i.id_insumo, i.nombre, i.categoria, i.unidad_medida, i.stock_actual, i.stock_minimo, p.razon_social FROM Insumos i LEFT JOIN Proveedores p ON p.id_proveedor=i.id_proveedor ORDER BY i.nombre`, r => ({ id: Number(r.id_insumo), name: text(r.nombre), category: text(r.categoria), unit: text(r.unidad_medida), currentStock: Number(r.stock_actual), minimumStock: Number(r.stock_minimo), supplierId: r.id_proveedor == null ? null : Number(r.id_proveedor), supplier: nullable(r.razon_social), stockStatus: Number(r.stock_actual) === 0 ? "out" : Number(r.stock_actual) <= Number(r.stock_minimo) ? "low" : "ok" })),
    suppliers: { async findAll(categories: string[] = []) { const where = categories.length ? ` WHERE categoria IN (${categories.map(() => "?").join(",")})` : ""; const [rows] = await pool.query<Row[]>(`SELECT id_proveedor, razon_social, categoria, contacto, telefono, correo, ultima_compra, estado FROM Proveedores${where} ORDER BY razon_social`, categories); return rows.map(r => ({ id: Number(r.id_proveedor), businessName: text(r.razon_social), category: text(r.categoria), contact: nullable(r.contacto), phone: nullable(r.telefono), email: nullable(r.correo), lastPurchase: nullable(r.ultima_compra), status: text(r.estado) })); } },
