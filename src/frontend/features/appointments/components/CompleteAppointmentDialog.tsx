...
type Completion = { observaciones: string; receta: string; recomendaciones: string };
...
export function CompleteAppointmentDialog({ open, pending, appointment, onOpenChange, onSubmit }: Props) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const completion = { observaciones: String(form.get("observaciones") ?? "").trim(), receta: String(form.get("receta") ?? "").trim(), recomendaciones: String(form.get("recomendaciones") ?? "").trim() };
